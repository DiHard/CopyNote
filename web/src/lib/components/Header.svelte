...
>
  ...
  <div class="flex shrink-0 items-center gap-0.5" style="-webkit-app-region: no-drag">
    <button
      ...
      onclick={openCreate}
      title={t("header.new")}
      class="rounded p-1 text-on-surface-dim transition hover:bg-surface-hover hover:text-on-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
    >
    ...
    <button
      ...
      class="relative rounded p-1 text-on-surface-dim transition hover:bg-surface-hover hover:text-on-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
    >
      <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      {#if hasUnseenUpdate()}
        <span
          class="absolute right-0.5 top-0.5 h-1.5 w-1.5 rounded-full bg-update-dot"
